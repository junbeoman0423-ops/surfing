...
# --- 메인 앱 로직 ---
def main():
    st.set_page_config(page_title="서핑 가자~~~!", page_icon="🏄", layout="wide")

    st.title("🌊 해양 기상 기반 서핑 환경 예측 시스템")
    st.markdown("Ocean ICT Festival 프로젝트 출품작 | SurfScience.com 2축 모델(파고 x 파도 형태) 기반 실시간 보드 추천")
    st.divider()

    # 대표 서핑/해수욕장 20곳: 위도, 경도, 해변 방위각(바다를 바라보는 방향, °)
    spots = {
        "양양 (죽도해수욕장)":       {"lat": 38.011, "lon": 128.761, "angle": 70},
        "양양 (기사문해변)":         {"lat": 38.030, "lon": 128.665, "angle": 75},
        "양양 (인구해변)":           {"lat": 38.046, "lon": 128.658, "angle": 75},
        "강릉 (금진해변)":           {"lat": 37.639, "lon": 129.196, "angle": 80},
        "강릉 (경포해변)":           {"lat": 37.805, "lon": 128.908, "angle": 75},
        "태안 (만리포해수욕장)":     {"lat": 36.782, "lon": 126.138, "angle": 260},
        "부산 (송정해수욕장)":       {"lat": 35.178, "lon": 129.199, "angle": 140},
        "부산 (다대포해수욕장)":     {"lat": 35.049, "lon": 128.966, "angle": 190},
        "제주 (중문해수욕장)":       {"lat": 33.244, "lon": 126.412, "angle": 180},
        "제주 (월정리해변)":         {"lat": 33.556, "lon": 126.796, "angle": 10},
        "제주 (이호테우해변)":       {"lat": 33.499, "lon": 126.463, "angle": 340},
        "제주 (삼양검은모래해변)":   {"lat": 33.520, "lon": 126.586, "angle": 5},
        "제주 (사계해변)":           {"lat": 33.225, "lon": 126.307, "angle": 190},
        "제주 (함덕해수욕장)":       {"lat": 33.543, "lon": 126.670, "angle": 15},
        "제주 (곽지해수욕장)":       {"lat": 33.450, "lon": 126.310, "angle": 320},
        "고성 (봉수대해수욕장)":     {"lat": 38.350, "lon": 128.470, "angle": 85},
        "포항 (신항만)":             {"lat": 36.070, "lon": 129.390, "angle": 95},
        "경주 (남열해돋이해수욕장)": {"lat": 35.767, "lon": 129.487, "angle": 95},
        "삼척 (용화해변)":           {"lat": 37.219, "lon": 129.313, "angle": 85},
        "삼척 (맹방해변)":           {"lat": 37.315, "lon": 129.198, "angle": 85},
    }

    if st.button("🌊 실시간 서핑 환경 데이터 분석 시작", type="primary"):
        ...
        st.subheader(f"🏆 오늘 가장 추천하는 서핑 스팟: **{best_spot['스팟']}** ({best_spot['종합 점수']}점)")

        col1, col2, col3, col4, col5 = st.columns(5)
        col1.metric("🌊 파고", f"{best_spot['파고 (ft)']} ft")
        col2.metric("⏱️ 파도 주기", f"{best_spot['파도 주기 (초)']} 초")
        col3.metric("🌊 파도 형태", best_spot['파도 형태'])
        col4.metric("🌡️ 기온", f"{best_spot['기온 (°C)']} °C")
        col5.metric("🏄 추천 보드", best_spot['추천 보드'])

        st.info(f"💡 **보드 선정 이유:** {best_spot['보드 설명']}")
        st.info(f"👗 **필수 준비물:** {best_spot['추천 준비물']}")

        # --- 종합 점수 상세 내역 (변수별 기여도) ---
        with st.expander(f"📊 {best_spot['스팟']} 점수 상세 내역 (총 {best_spot['종합 점수']}점 / 100점)"):
        ...
        with col_table:
            st.markdown("### ​지역별 상세 분석 결과 (변수별 기여 점수 포함)")
